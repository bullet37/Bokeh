...
class Attention(nn.Module):
    ...
    def __init__(self, dim, group_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        if position_bias:
            ...
            biases = biases.flatten(1).transpose(0, 1).float() # 169 2
...
class CrossFormerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.lsda_flag == 0:  # 0 for SDA
            x = x.reshape(B, H // G, G, W // G, G, C).permute(0, 1, 3, 2, 4, 5) # 2,8,8,7,7,96
        else:  # 1 for LDA
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x): # 2, 3, 224, 224
        ...
        for i in range(len(self.projs)):
            tx = self.projs[i](x).flatten(2).transpose(1, 2)  #2, 3136, 96
...
class CrossFormer(nn.Module):
    ...
    def forward(self, x):
        x = self.forward_features(x)  # 2 768
...
if __name__ == '__main__':
    ...
    model = CrossFormer().cpu()
    params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(params * 1e-6, 'M')

    x = torch.rand(2, 3, 224, 224).cpu()
